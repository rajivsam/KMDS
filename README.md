...
KMDS was originally designed for data scientists writing Python. Recent additions to the CLI, interactive UI layouts, and natural-language tooling mean it is now practical for a broader set of users:
...
| **Business analyst**   | Interactive UI Dashboard, CLI commands, and natural-language ingestion |
...
- **Interactive UI Dashboard:** Ingest, inspect, edit, and export your KMDS knowledge base via an enterprise single-page web interface.
...
Install the core KMDS engine along with the dedicated UI package directly from PyPI:

```bash
# Install core knowledge capture framework
pip install kmds

# Install the interactive visual application layout extension
pip install kmds-ui
```

### 2. Launching the Interactive UI

Once installed, you can spin up the workbench app directly from your terminal using the system console script entry point:

```bash
kmds-workbench
```

*Open `http://127.0.0` inside your browser window to begin interacting with your knowledge base.*
...
Ask a free-form question. The orchestrator routes it to the best KMDS observation-query template using an LLM, executes the template, and synthesizes a plain-English answer. Falls back to semantic search automatically.
